...
def predict_image(image_path):
    ...
    model = tf.keras.models.load_model("image_calssifier.h5")
    ...
    if class_mode == "binary":
        predicted_class = class_names[int(bool(prediction > 0.5))]
    else:
        predicted_class = class_names[tf.argmax(prediction, axis=-1).numpy()]

    print(f"Модель определила: {predicted_class}")

predict_image("doges.jpeg")
